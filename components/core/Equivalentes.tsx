...
export default function Equivalentes() {
	const DisciplinasEquivalentes = useDisciplinaStore(
		(state) => state.DisciplinasEquivalentes,
	);
	...
	const getDisciplinaByName = useDisciplinaStore(
		(state) => state.getDisciplinaByName,
	);
	...
	const DisciplinasVisiveis = useMemo(() => {
		...
		const grupos = Array.from(gruposMap.entries()).map(
			([equivaleNome, disciplinas]) => {
		...
		<>
			{mode !== "minimal" && (
				<SectionHeader title="Disciplinas Equivalentes">
					<p>
						Nessa seção está presentes todas as disciplinas equivalentes até o
						momento, pode ser que algumas no futuro não sejam mais aceitas
						porque sairão do fluxo esperado.
					</p>
				</SectionHeader>
			)}
			...
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
				{DisciplinasVisiveis.map((grupo) => {
					const { cardClasses, titleClasses } = generateDisciplinaClasses(
						grupo.disciplinas[0].equivaleId,
					);
					return (
						<section key={grupo.equivaleNome} className={`mb-8 ${cardClasses}`}>
							<h3
								className={`text-xl font-bold mb-4 border-b-2 border-gray-200 pb-2 ${titleClasses}`}
							>
