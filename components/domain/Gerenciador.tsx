									...
									return (
										...
										>
											...
											<div className="flex justify-center flex-col mt-3">
												{disciplina.equivalentes && disciplina.equivalentes.length > 0 && (
													<>
														<span
															className="cursor-pointer py-1 text-xs bg-blue-600 hover:bg-blue-500 rounded-full text-white w-1/2 text-center"
															onClick={(e) => {
																e.stopPropagation();
																setExpandEquivalente(disciplina.id !== expandEquivalente ? disciplina.id : undefined);
															}}
														>
															{expandEquivalente === disciplina.id ? "Esconder equivalentes" : "Ver equivalentes"}
														</span>
														{expandEquivalente === disciplina.id && (
															<span className="text-xs mt-2 font-bold flex-col flex gap-1">
																Equivalentes:
																<div className="flex flex-col gap-2 font-normal">
																	{disciplina.equivalentes.map((equivalente, index) => (
																		<span key={index}>
																			<span className="font-semibold">{equivalente.curso}</span>
